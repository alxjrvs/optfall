...
- **Thirteen primitives** — pitch jewel, bevelled plate, notched state pill,
  brass seal, citation, filigree corner, ornamental rule, the mark, card face,
  search field, result row, stat glyph and game symbol. axe-core runs over every
  one of them in CI, and a check fails if a primitive is added without a case.
...
bun run check:full     # the whole gate, including the 13,676-page build (~3 min)

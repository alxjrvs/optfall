...
- **Fifteen primitives** — pitch jewel, pitch rule, bevelled plate, notched
  state pill, brass seal, citation, filigree corner, ornamental rule, the mark,
  card face, search field, result row, stat glyph, game symbol and pagination.
  axe-core runs over every one of them in CI, and a check fails if a primitive
  is added without a case. The list of record is `PRIMITIVES` in
  `packages/components/src/index.ts`; this prose has been wrong twice, so prefer
  it over this sentence.

...
cards.push({
  ...
  <p class="note"><strong>Pitch four is one card, and the corpus has not caught up to it.</strong> Legend Story Studios printed a fourth value — a purple strip on a Shadow resource gem — and the pinned card data holds none of them yet, so this stone is drawn here before it is drawn anywhere in the product. It sits beside the other four rather than being kept as a special case, because a stone the reader meets once is the one whose colour they will not have learned. It is also the stone whose colour channel is weakest: purple sits beside blue in luminance wherever it is still legible under white ink, so for a reader with deuteranopia or protanopia the numeral is what separates three from four.</p>`,
});

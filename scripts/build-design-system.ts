...
cards.push({
  ...
  <p class="note" style="margin-block-start:var(--of-space-loose)"><strong>Two tones, and it shipped with three.</strong> <code>ink-muted</code> and full <code>ink</code>, in that order above. The difference between them is how loudly the label competes with what it labels, not a palette choice &mdash; which is why they are named for their loudness rather than exposed as a colour prop. A colour prop would put a raw value back in a page, and <code>check:tokens</code> would fail the build; correct, but a slow way to learn it.</p>
  <p class="note"><strong>The third tone was <code>ink-faint</code>, it was the default, and removing it is the point.</strong> The survey counted three inks across the twenty-eight blocks and this component reproduced all three &mdash; but <code>color.ink.faint</code> is the LARGE-text ink, held to 3:1 rather than 4.5:1 on purpose, and an eyebrow is <code>type.size.micro</code> by definition. Most of the surveyed blocks were already below AA, so making the majority the default turned a defect repeated in ten page stylesheets into one every caller inherited. <code>check:faint-ink</code> named this file first. A tone no caller may correctly pass is a trap rather than an option, so it is gone from the union rather than merely demoted &mdash; and gone is what the gate cannot enforce by itself, since it matches a colour and a size in the same rule and a <code>data-tone</code> rule carries no size.</p>

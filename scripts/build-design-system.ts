...
cards.push({
  ...
  <p class="note"><strong>Pitch four is one card, and the corpus has not caught up to it.</strong> A fourth value has been previewed — a purple strip on a Shadow resource gem, card code <code>IAR000</code> — and the pinned data holds no card from that set, which it still records as "??? Set 20 ???". So this stone is drawn here before it is drawn anywhere in the product. It sits beside the other four rather than being kept as a special case, because a stone the reader meets once is the one whose colour they will not have learned. It is also the stone whose colour channel is weakest: purple sits beside blue in luminance wherever it is still legible under white ink, so for a reader with deuteranopia or protanopia the numeral is what separates three from four.</p>`,
});

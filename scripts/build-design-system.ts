...
cards.push({
  ...
  <p class="note" style="margin-block-start:var(--of-space-tight)">1,648 cards print a cost of 0, 191 a defence of 0 and 13 a power of 0, so both members of each pair above are real and a reader has to be able to tell them apart. <strong>An empty position paints nothing and keeps its width.</strong> The gap you can see above is the mark, rendered and hidden — the corner has to hold the width a printed value would have taken, or the name and the type line slide off the card's axis. Nothing is drawn in the value position either: an en dash sat there until the values moved out of the plates, and beside a mark, where every printed value now is, a dash reads as something the card prints.</p>
  <p class="note"><strong>This is the one thing the drawn plates did better, and two answers were tried before this one.</strong> An absence used to be a recessed fill — a token, in the theme, checked like every other colour — with the silhouette still saying WHICH stat was missing. An ingested PNG takes no token, so the mark was desaturated and dropped to 30% instead; on a page that read as a rendering fault rather than a statement, at about 1.3:1 against the panel. So the position is simply not drawn, and the claim moved to the accessible name — which the card panel then hides, because a stat the card does not print is not a fact the card states. That is the price of rendering the game's own file, and it is stated here rather than discovered.</p>
